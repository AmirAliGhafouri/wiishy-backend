...
{
    public static function count($user_id ,$item)
    {
        return User::where('id' ,$user_id)->first()->$item;
    }

    public static function follow_list($user_id, $join, $field)
    {
    ...
    public static function list($user_id, $join, $field, $my_id)
    {
        return DB::table('users')
        ->join('userfollows', 'users.id', '=', $join)
        ->where([$field => $user_id, 'follow_status' => 1])
    ...
    public static function unique($fooloweings, $followers)
    {
        $suggestions = array_merge($fooloweings, $followers);
        return array_unique($suggestions);
    }

    public static function filter($suggestions, $my_id)
    {
        $users = array();
    ...
    public static function follow_suggestion($users)
    {
        $user_details = array();
        foreach($users as $user) {
    ...
    public static function check($user_id, $follow_id)
    {
    ...
    public static function follow($user_id, $follow_id)
    {
    ...
    public static function unfollow($user_id, $follow_id)
    {
